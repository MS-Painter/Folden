...
impl HandlerService for Server {
    ...
    async fn register_to_directory(
        &self,
        request: Request<generated_types::RegisterToDirectoryRequest>,
            ...
            .is_some()
        {
            ...
            if request.is_start_on_register {
                ...
                match mapping.spawn_handler_thread(
    ...
    async fn get_directory_status(
        &self,
        request: Request<generated_types::GetDirectoryStatusRequest>,
    ) -> Result<Response<HandlerSummaryMapResponse>, tonic::Status> {
    ...
    async fn start_handler(
        &self,
        request: Request<generated_types::StartHandlerRequest>,
    ) -> Result<Response<HandlerStatesMapResponse>, tonic::Status> {
        ...
        match mapping.clone().directory_mapping.get_mut(directory_path) {
            Some(handler_mapping) => {
                if !handler_mapping.is_alive()
                    && self.is_concurrent_handlers_limit_reached(&mapping)
                {
            ...
            None => {
                ...
                if request.directory_path.is_empty() {
                    ...
                    for (directory_path, handler_mapping) in
                        mapping.clone().directory_mapping.iter_mut()
                    {
                        let trace_tx = self.handlers_trace_tx.clone();
                        let response =
                            mapping.start_handler(directory_path, handler_mapping, trace_tx);
                        states_map.insert(directory_path.to_owned(), response);
                    }
                } else {
                    states_map.insert(
    ...
    async fn stop_handler(
        &self,
        request: Request<generated_types::StopHandlerRequest>,
    ) -> Result<Response<HandlerStatesMapResponse>, tonic::Status> {
    ...
    async fn modify_handler(
        &self,
        request: Request<generated_types::ModifyHandlerRequest>,
    ...
    async fn trace_handler(
        &self,
        request: Request<generated_types::TraceHandlerRequest>,
    ) -> Result<Response<Self::TraceHandlerStream>, tonic::Status> {
        ...
        tracing::debug!(
            ...
            self.handlers_trace_tx.receiver_count()
        );
