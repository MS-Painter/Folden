...
impl ServiceEndpoint<Request, Response> for StarthandlerEndpoint<'_> {
    fn execute(&self) -> Result<Response, tonic::Status> {
        ...
        match self.mapping.clone().directory_mapping.get_mut(directory_path) {
            Some(handler_mapping) => {
                if !handler_mapping.is_alive()
                    && self.server.is_concurrent_handlers_limit_reached(&self.mapping)
                {
                ...
                let response = self.mapping.start_handler(directory_path, handler_mapping, trace_tx);
            ...
            None => {
                ...
                if request.directory_path.is_empty() {
                    if self.mapping.directory_mapping.len() > self.server.config.concurrent_threads_limit.into()
                    {
                        ...
                        self.mapping.clone().directory_mapping.iter_mut()
                    {
                        let trace_tx = self.server.handlers_trace_tx.clone();
                        let response =
                            self.mapping.start_handler(directory_path, handler_mapping, trace_tx);
                        states_map.insert(directory_path.to_owned(), response);
                    }
                } else {
                    states_map.insert(
