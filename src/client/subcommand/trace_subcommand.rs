...
use super::subcommand_utils::{
    construct_directory_or_all_args, get_path_from_matches_or_current_path, SubCommandUtil,
};
use folden::shared_utils::construct_port_arg;
use generated_types::{
    handler_service_client::HandlerServiceClient, GetDirectoryStatusRequest, TraceHandlerRequest,
};
...
fn print_response(response: generated_types::TraceHandlerResponse, print_directory: bool) {
    if print_directory {
        ...
        Message - {}
        ",
            response.directory_path,
            response.action.unwrap_or_else(|| "None".to_string()),
            response.message
        );
    } else {
        println!(
